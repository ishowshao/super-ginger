...
Popup.prototype._initHeader = function () {
    var game = this.game;
    var header = game.add.image(-this.width / 2, game.cache.getImage('popup-header').height / 2, 'popup-header');
    header.anchor.set(0, 0.5);
    this.body.addChild(header);
    this.header = header;
    var padding = 12;
    var titleText = game.add.text(padding, 3, this.title, {
    ...
    case 'food':
        ...
        var foodCountText = game.add.text(food.x - food.width - 10, 3, global.getFoodCount() + '', {
...
Popup.prototype._initContainer = function () {
    ...
    var container = game.add.image((game.width - this.width) / 2 + margin, game.height + this.paddingTop);
...
Popup.prototype._initCrop = function () {
    ...
    crop.beginFill(16777215);
    crop.drawRect(container.x, game.height + this.paddingTop, this.width - margin * 2, this.containerHeight);
    crop.endFill();
    container.mask = crop;
...
Popup.prototype._show = function () {
    ...
    this.elements.forEach(function (el) {
        game.add.tween(el).to({ y: y - game.height + '' }, 600, Phaser.Easing.Back.Out, true);
    });
};
Popup.prototype._hide = function (tweenEnabled) {
    ...
    if (tweenEnabled) {
        this.elements.forEach(function (el) {
            var move = game.add.tween(el).to({ y: game.height - y + '' }, 600, Phaser.Easing.Back.In);
