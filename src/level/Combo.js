...
Combo.prototype._init = function () {
    ...
    body.alpha = 0;
    this.body = body;
    var label = game.add.text(0, 0, 'COMBO', {
    ...
    body.addChild(label);
    var multipleSign = game.add.text(label.width + 5, 0, '\xD7', {
        font: 'bold 36px ' + global.fontFamily,
        fill: color.get('white')
    });
    multipleSign.anchor.set(0, 0.5);
    body.addChild(multipleSign);
    var countText = game.add.text(multipleSign.x + multipleSign.width + 18, 0, this.count + '', {
        font: 'bold 36px ' + global.fontFamily,
...
Combo.prototype._show = function () {
    this.game.add.tween(this.body).to({ alpha: 1 }, 300, Phaser.Easing.Quadratic.Out, true);
};
Combo.prototype._hide = function (cb) {
    this.game.add.tween(this.body).to({ alpha: 0 }, 300, Phaser.Easing.Quadratic.In, true);
};
Combo.prototype.add = function () {
    ...
    var largen = game.add.tween(countText.scale).to({
        x: 1.5,
        y: 1.5
    }, duration, easingQuadratic.Out);
    var recover = game.add.tween(countText.scale).to({
        x: 1,
        y: 1
    }, duration, easingQuadratic.In);
