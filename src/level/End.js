...
End.prototype._initTitle = function () {
    var title = this.game.add.text(0, 0, '结束啦', {
...
End.prototype._initBoard = function () {
    ...
    var scoreTitle = game.add.text(0, 15, '分数', titleFontStyle);
    scoreTitle.anchor.set(0.5, 0);
    board.addChild(scoreTitle);
    var highestTitle = game.add.text(0, 108, '最佳', titleFontStyle);
    ...
    var scoreValue = game.add.text(0, 45, this.score + '', valueFontStyle);
    scoreValue.anchor.set(0.5, 0);
    board.addChild(scoreValue);
    var hightest = global.getHighest();
    var highestValue = game.add.text(0, 138, hightest + '', valueFontStyle);
    highestValue.anchor.set(0.5, 0);
    board.addChild(highestValue);
    if (this.hasNewRecord) {
        var newRecord = game.add.image(scoreTitle.x + scoreTitle.width / 2 + 18, scoreTitle.y, 'new-record');
        board.addChild(newRecord);
        var newRecordText = game.add.text(newRecord.x + newRecord.width / 2, newRecord.y + 16, '新纪录', {
...
End.prototype._initBtns = function () {
    ...
    var btnConfigs = [
        {
            ...
            onClick: function () {
                ...
                mask.show(duration);
                var tipText = game.add.text(game.width / 2, 200, '点击右上角\n分享到朋友圈', {
                ...
                game.add.tween(tipText).from({ alpha: 0 }, duration, this.ease, true);
                setTimeout(function () {
                    var duration = 400;
                    mask.hide(duration);
                    var hideTip = game.add.tween(tipText).to({ alpha: 0 }, duration, this.ease);
    ...
    btnConfigs.forEach(function (config, index) {
        ...
        util.addHover(btn);
        body.addChild(btn);
        var text = game.add.text(0, 3, config.text, {
...
End.prototype._initTip = function () {
    ...
    var text = game.add.text(8, 7, '换个角色\n试试', {
        font: 'bold 20px ' + global.fontFamily,
        fill: color.get('white'),
        align: 'center'
    });
    tip.addChild(text);
    game.add.tween(tip).to({ y: '-5' }, 1000, Phaser.Easing.Sinusoidal.InOut, true, 0, -1, true);
};
End.prototype._show = function () {
    var duration = 500;
    this.mask.show(duration);
    this.game.add.tween(this.body).from({ alpha: 0 }, duration, Phaser.Easing.Quadratic.InOut, true);
};
End.prototype._updateShare = function () {
    global.setShareText('我的\u300E' + global.getHeroConfig().chName + '\u300F勇闯了' + this.score + '道甜点关\uFF0C你也来试试吧\uFF01');
