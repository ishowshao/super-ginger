...
Foodboard.prototype._init = function () {
    var game = this.game;
    var text = game.add.text(game.width - 63, 20, global.getFoodCount() + '', {
...
Foodboard.prototype.update = function () {
    ...
    var largen = game.add.tween(text.scale).to({
        x: 1.2,
        y: 1.2
    }, duration, easingQuadratic.Out);
    var recover = game.add.tween(text.scale).to({
        x: 1,
        y: 1
