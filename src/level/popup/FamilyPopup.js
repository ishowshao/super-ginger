...
FamilyPopup.prototype._initPanel = function (config, index) {
    ...
    var container = this.container;
    var panel = this.panel;
    var topEdge = game.add.image(0, (panel.marginTop + panel.height) * index + panel.marginTop, 'panel-edge');
    ...
    var dividing = game.add.image(panel.dividingX, topEdge.y, 'pixel-beige');
...
FamilyPopup.prototype._initHero = function (config, index, options) {
    var game = this.game;
    var panel = this.panel;
    var hero = game.add.sprite(47 + config.paddingRight, panel.mainHeight / 2 - 2, config.name + '-down');
...
FamilyPopup.prototype._initInfo = function (config, index, options) {
    ...
    var nameText = game.add.text(panel.dividingX / 2, panel.mainHeight - 18, config.chName, {
    ...
    var descTitleText = game.add.text(panel.dividingX + panel.dividingWidth + 10, 10, '简介', titleFontStyle);
    panelCtn.addChild(descTitleText);
    var contentFontStyle = {
        font: '17px ' + global.fontFamily,
        fill: color.get('darker-grey')
    };
    var descText = game.add.text(descTitleText.x + descTitleText.width + 6, descTitleText.y + 4, config.desc, contentFontStyle);
    panelCtn.addChild(descText);
    var powerTilteText = game.add.text(panel.dividingX + panel.dividingWidth + 10, descText.y + descText.height + 5, '超能', titleFontStyle);
    panelCtn.addChild(powerTilteText);
    var powerText = game.add.text(powerTilteText.x + powerTilteText.width + 6, powerTilteText.y + 4, global.getUnlock(index) ? config.powerText : '? ? ?', contentFontStyle);
...
FamilyPopup.prototype._initBtnSelect = function (config, index, options) {
    if (global.getUnlock(index)) {
        var game = this.game;
        var panel = this.panel;
        var btnSelect = game.add.button(0, options.y, 'pixel-black', function () {
            this._select(index);
            this._hide();
        }, this);
...
FamilyPopup.prototype._initBtnUnlock = function (config, index, options) {
    if (!global.getUnlock(index)) {
        ...
        var panel = this.panel;
        var me = this;
        var btnUnlock = game.add.button(panel.dividingX + panel.dividingWidth, options.y, 'btn-unlock', function () {
            ...
            setTimeout(function () {
                ...
                if (global.getFoodCount() >= config.cost) {
                    ...
                    new Confirm(game, {
                        ...
                        onConfirm: function () {
                            global.setFoodCount(global.getFoodCount() - config.cost, function () {
                                global.unlock(index);
                                me._select(index);
                            });
                        }
        ...
        case 'food':
            var costText = game.add.text(btnUnlock.width / 2 - 8, btnUnlock.height / 2, config.cost, {
                font: 'bold 36px ' + global.fontFamily,
                fill: color.get('white')
            });
            costText.anchor.set(1, 0.5);
            btnUnlock.addChild(costText);
            var food = game.add.image(btnUnlock.width / 2 + 8, btnUnlock.height / 2 - 5, 'food');
            ...
            break;
        case 'share':
            var shareText = game.add.text(btnUnlock.width / 2, btnUnlock.height / 2, '分享即可获得', {
...
FamilyPopup.prototype._initPager = function () {
    ...
    var btnDown = game.add.button(game.width / 2 + margin, y, 'btn-down', function () {
        if (this.downActive) {
            this._turnPage('down');
        }
    }, this);
    ...
    var btnUp = game.add.button(game.width / 2 - margin - btnDown.width, y, 'btn-up', function () {
        if (this.upActive) {
            this._turnPage('up');
        }
    }, this);
    ...
    [
        ...
        events.onInputDown.add(function (target) {
            if (me[activeKey]) {
                target.alpha = 0.85;
            }
        }, me);
        events.onInputUp.add(function (target) {
            if (me[activeKey]) {
                target.alpha = 1;
            }
        }, me);
...
FamilyPopup.prototype._turnPage = function (type) {
    ...
    var turn = this.game.add.tween(this.container).to({ y: offset + '' }, 200, Phaser.Easing.Sinusoidal.InOut);
    turn.onStart.add(function () {
        this.upActive = false;
