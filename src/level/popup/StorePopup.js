...
StorePopup.prototype._initContent = function () {
    ...
    storeData.forEach(function (data, index) {
        var btn = game.add.button(0, (panel.marginTop + panel.height) * index + panel.marginTop, 'transparent-2', function () {
            window.open('http://pay.yiluwan.org/pay-center/index.php?' + 'item=000' + (index + 6) + '&redirect=' + href);
        });
        util.addHover(btn);
        ...
        btn.addChild(food);
        var numberText = game.add.text(210, panel.height / 2, '+' + data.food, {
        ...
        var moneyText = game.add.text(panel.width - 30, panel.height / 2, '\uFFE5 ' + data.money, {
