...
RankPopup.prototype._initRows = function (rankData) {
    ...
    rankData.forEach(function (data, index) {
        var row = game.add.image(0, marginTop + rowHeight * index, index % 2 === 0 ? 'pixel-dark-beige' : 'pixel-black');
        if (index % 2 === 0) {
        ...
        var rankText = game.add.text(15, rowHeight / 2, data.rank ? data.rank > 9999 ? '9999+' : data.rank : index + 1, {
        ...
        if (data.rank && data.rank > 99) {
            rankText.scale.set(60 / rankText.width);
        }
        rowCtn.addChild(rankText);
        var nameText = game.add.text(85, rowHeight / 2, data.nickname, {
        ...
        if (data.nickname === myNickname) {
            var meTip = game.add.image(nameText.x + nameText.width + 10, rowHeight / 2, 'me-tip');
            meTip.anchor.set(0, 0.5);
            rowCtn.addChild(meTip);
            var meText = game.add.text(meTip.width - 5, 0, 'me', {
        ...
        var scoreText = game.add.text(rowWidth - 15, rowHeight / 2, data.highest + '', {
